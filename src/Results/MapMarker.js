...
class MapMarker extends Component {
    ...
    componentDidMount = async () => {
        ...
        let location = [] ;
          ...
          let re = /([[:blank]]|(&))/g ; 
          let el = this.props.location.replace(re, '+');
    
            axios.get(`https://api.opencagedata.com/geocode/v1/json?q=${el}+sanfrancisco+us&key=${ApiKey}`)
          .then( geo => {
    ...
    render() {
        ...
        return(
            ...
            {isLoaded && 
             <Marker className="Icon" position={[locationCoord[0], locationCoord[1]]} icon={myIcon}>
